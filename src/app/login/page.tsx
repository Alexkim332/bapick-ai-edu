...
function LoginFormContent() {
  ...
  const isSignUpSuccess = searchParams.get('success') === 'true';
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 sm:px-6 lg:px-8 bg-background-light min-h-[calc(100vh-4rem)]">
      <div className="w-full max-w-md bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="p-8">
          {isSignUpSuccess ? (
            <div className="bg-green-50 border border-green-200 text-green-800 p-6 rounded-xl text-center flex flex-col items-center justify-center gap-4">
              <h3 className="text-xl font-bold">🎉 회원가입 메일이 발송되었습니다!</h3>
              <p className="text-sm">
                입력하신 이메일의 수신함(또는 스팸함)을 확인하여 인증 링크를 클릭해 주세요. 인증이 완료되어야 로그인이 가능합니다.
              </p>
              <Link 
                href="/login"
                className="mt-4 inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 shadow-sm transition-colors"
                onClick={() => setActiveTab('login')}
              >
                로그인 화면으로 돌아가기
              </Link>
